"""
Core JIDT analysis entry points (English-only).

Provides JVM lifecycle helpers and wrappers that invoke JIDT via JPype.
Functions use JSDoc-style, DbC-oriented docstrings to clarify contracts.
...
def start_jvm(xms: str = "8g", xmx: str = "16g", gc_opts=None):
    """
    Start the JPype JVM with the JIDT classpath from settings.

    @param {str} xms - Initial heap size (e.g., "8g").
    @param {str} xmx - Maximum heap size (e.g., "16g").
    @param {list|null} gc_opts - Additional GC options (default: ["-XX:+UseG1GC","-XX:MaxGCPauseMillis=200"]).
    @returns {None}
    @throws {ValueError} If `settings.JIDT_JAR_PATH` is invalid.
    @throws {RuntimeError} If starting the JVM fails.
    @pre JVM not already started, or safe to be idempotent.
    @post JVM started with JIDT on classpath; function is idempotent.
    """
    global _jvm_started
...
def shutdown_jvm():
    """
    Shut down the JPype JVM if previously started.

    @returns {None}
    @post JVM is not running after this call.
    """
    global _jvm_started
...
def get_jidt_classes() -> dict:
    """
    Import and return required JIDT Java classes (lazy-load).

    @returns {dict} Mapping of class aliases to JIDT class objects.
    @throws {Exception} If classes cannot be imported (e.g., missing jar).
    @pre JVM started or startable; `settings.JIDT_JAR_PATH` valid.
    @post Returns a cached mapping on subsequent calls.
    """
...
def java_array_int(py_array: np.ndarray) -> JArray:
    """
    Convert a numpy integer array to a Java int[] for JIDT.

    @param {np.ndarray|Sequence[int]} py_array - Integer array.
    @returns {JArray} Java int[] with same contents.
    @throws {TypeError} If input is not integer-typed.
    @pre Input data type is integer.
    @post Returns a 1-D Java array of equal length.
    """
...
def find_optimal_k_ais(series: np.ndarray, base: int, max_k: int) -> int:
    """
    Select the history length k that maximizes AIS over k ∈ [1..max_k].

    @param {np.ndarray} series - Discretized series (int32) with symbols in [0, base-1].
    @param {int} base - Alphabet size (>1 recommended).
    @param {int} max_k - Maximum k to evaluate.
    @returns {int} Best k by AIS, or 1 if no valid AIS could be computed.
    @pre len(series) > 0 and 1 <= max_k.
    @post Returns an integer in [1..max_k].
    """
    if base <= 1:
...
def run_te_analysis(series_A: np.ndarray, series_S: np.ndarray,
    ...
    """
    Compute TE(A→S) and TE(S→A) with p-values via JIDT Discrete TE.

    @param {np.ndarray} series_A - Discretized A with base_A.
    @param {np.ndarray} series_S - Discretized S with base_S.
    @param {int} k_A - History for source A.
    @param {int} k_S - History for destination S.
    @param {int} base_A - Alphabet size for A.
    @param {int} base_S - Alphabet size for S.
    @param {int} tau - Delay parameter.
    @param {int} num_surrogates - Surrogates for significance when not adaptive.
    @param {list|null} adaptive_stages - Optional staged surrogate counts.
    @param {float|null} early_stop_sig - Early stop p-threshold for significance.
    @param {float|null} early_stop_nonsig - Early stop p-threshold for non-significance.
    @returns {dict} {'TE(A->S)', 'p(A->S)', 'TE(S->A)', 'p(S->A)', 'Delta_TE'}
    @pre len(series_A) == len(series_S) > 0.
    @post Returns NaN values on failures; does not raise unless validation fails internally.
    """
...
def run_cte_analysis(series_A: np.ndarray, series_S: np.ndarray, series_cond: np.ndarray,
                     ...
                     adaptive_stages=None, early_stop_sig: float = None, early_stop_nonsig: float = None) -> dict:
    """
    Compute stratified CTE(A→S|H) and CTE(S→A|H); kept for comparison.

    @param {np.ndarray} series_A - Discretized A.
    @param {np.ndarray} series_S - Discretized S.
    @param {np.ndarray} series_cond - Hour-of-day bins.
    @param {int} k_A - History for A.
    @param {int} k_S - History for S.
    @param {int} base_A - Alphabet size for A.
    @param {int} base_S - Alphabet size for S.
    @param {int} base_cond - Alphabet size for H.
    @param {int} tau - Delay parameter.
    @param {int} num_surrogates - Surrogates for significance.
    @returns {dict} {'CTE(A->S|H_bin)', 'CTE(S->A|H_bin)', p-values, 'Delta_CTE_bin'}
    @deprecated Prefer run_true_cte_analysis().
    @pre All arrays aligned and of equal length.
    @post Returns NaN values on failure; no side effects.
    """
...
def run_true_cte_analysis(series_A: np.ndarray, series_S: np.ndarray, series_cond: np.ndarray,
                          ...
                          adaptive_stages=None, early_stop_sig: float = None, early_stop_nonsig: float = None) -> dict:
    """
    Compute True CTE(A→S|H) and CTE(S→A|H) via JIDT discrete conditional TE.

    @param {np.ndarray} series_A - Discretized A with base_A.
    @param {np.ndarray} series_S - Discretized S with base_S.
    @param {np.ndarray} series_cond - Hour-of-day bins with base_cond.
    @param {int} k_A - History for A.
    @param {int} k_S - History for S.
    @param {int} base_A - Alphabet size for A.
    @param {int} base_S - Alphabet size for S.
    ...
    @param {float|null} early_stop_sig - Early stop p-threshold for significance.
    @param {float|null} early_stop_nonsig - Early stop p-threshold for non-significance.
    @returns {dict} {'CTE_true(A->S|H)', 'p_true_cte(A->S|H)', 'CTE_true(S->A|H)', 'p_true_cte(S->A|H)', 'Delta_CTE_true'}
    @pre len(series_A) == len(series_S) == len(series_cond) > 0; symbols within bases.
    @post Returns NaN values on failure; arrays GC’d internally.
    """
